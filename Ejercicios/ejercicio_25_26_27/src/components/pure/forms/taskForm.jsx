...
const TaskForm = ({ add, filter, length }) => {
      ...
      <Formik initialValues={initialValues} validationSchema={taskSchema} onSubmit={addTask}>
        {({ values, touched, errors, isSubmitting, handleChange, handleBlur }) => (
          <Form>
            ...
            <div>
              <button type="button" onClick={() => filter(LEVELS.NORMAL)}>
                NORMAL
              </button>
              <button type="button" onClick={() => filter(LEVELS.URGENT)}>
                URGENT
              </button>
              <button type="button" onClick={() => filter(LEVELS.BLOCKING)}>
                BLOCKING
              </button>
            </div>
          </Form>
